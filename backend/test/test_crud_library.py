...
class TestCrudLibrary:
    ...
    def test_library_flow_add_update_rate_list_remove(self, crud_base_url):
        user_id = 123
        ...
        # set rating
        r = requests.put(
            f"{crud_base_url}/api/books/{book_id}/rate",
            json={"rating": 5},
            headers=headers,
            timeout=5,
        )
    ...
    def test_invalid_rating_returns_400(self, crud_base_url):
        user_id = 456
        headers = {"X-User-Id": str(user_id)}
        book_id = self._get_first_book_id(crud_base_url)

        # must exist first
        r = requests.post(f"{crud_base_url}/api/books/{book_id}/library", headers=headers, timeout=5)
        assert r.status_code == 200

        r = requests.put(f"{crud_base_url}/api/books/{book_id}/rate", json={"rating": 0}, headers=headers, timeout=5)
